...
public class XmlEditor {
    ...
    public static String[] isNotAppTag = new String[] { "uses-permission", "uses-sdk", "compatible-screens",
            "instrumentation", "library", "original-package", "package-verifier", "permission", "permission-group",
            "permission-tree", "protected-broadcast", "resource-overlay", "supports-input", "supports-screens",
            "upgrade-key-set", "uses-configuration", "uses-feature" };
                ...
                for (AttributeData attrData : tag.startTagChunk.attrList) {
                    ...
                    if ("name".equals(attrName)) {
                        ...
                        if (name.equals(value)) {
                            ...
                            ParserChunkUtils.xmlStruct.byteSrc = Utils.removeByte(ParserChunkUtils.xmlStruct.byteSrc,
                                    delStart, delSize);
            ...
            while (event != XmlPullParser.END_DOCUMENT) { // 文档结束
                ...
                case XmlPullParser.START_TAG: // 标签开始
                    ...
                    for (int i = 0; i < pullParser.getAttributeCount(); i++) {
                        ...
                        int attrtype = type[0];
                        int attrdata = type[1];
                        AttributeData data = AttributeData.createAttribute(attruri, attrname, attrvalue, attrtype,
                                attrdata);
                    ...
                    if (isNotAppTag(tagName)) {
                        ParserChunkUtils.xmlStruct.byteSrc = Utils.insertByte(ParserChunkUtils.xmlStruct.byteSrc,
                                subTagChunkOffsets, startChunk.getChunkByte());
                        subTagChunkOffsets += startChunk.getChunkByte().length;
                    } else {
                        ParserChunkUtils.xmlStruct.byteSrc = Utils.insertByte(ParserChunkUtils.xmlStruct.byteSrc,
                                subAppTagChunkOffset, startChunk.getChunkByte());
                ...
                case XmlPullParser.END_TAG: // 标签结束
                    ...
                    if (isNotAppTag(tagName)) {
                        ParserChunkUtils.xmlStruct.byteSrc = Utils.insertByte(ParserChunkUtils.xmlStruct.byteSrc,
                                subTagChunkOffsets, endChunk.getChunkByte());
                        subTagChunkOffsets += endChunk.getChunkByte().length;
                    } else {
                        ParserChunkUtils.xmlStruct.byteSrc = Utils.insertByte(ParserChunkUtils.xmlStruct.byteSrc,
                                subAppTagChunkOffset, endChunk.getChunkByte());
                        ...
                        if (tagName.equals(value)) {
                            for (AttributeData data : chunk.attrList) {
                                ...
                                if (attrName.equals(attrNameTemp1)) {
                                    ...
                                    int countStart = chunk.offset + 28;
                                    byte[] modifyByte = Utils.int2Byte(chunk.attrList.size() - 1);
                                    ParserChunkUtils.xmlStruct.byteSrc = Utils
                                            .replaceBytes(ParserChunkUtils.xmlStruct.byteSrc, modifyByte, countStart);

                                    // 修改chunk的大小
                                    int chunkSizeStart = chunk.offset + 4;
                                    int chunkSize = Utils.byte2int(chunk.size);
                                    byte[] modifyByteSize = Utils.int2Byte(chunkSize - 20);
                                    ParserChunkUtils.xmlStruct.byteSrc = Utils.replaceBytes(
                                            ParserChunkUtils.xmlStruct.byteSrc, modifyByteSize, chunkSizeStart);

                                    // 删除属性内容
                                    int delStart = data.offset;
                                    int delSize = data.getLen();
                                    ParserChunkUtils.xmlStruct.byteSrc = Utils
                                            .removeByte(ParserChunkUtils.xmlStruct.byteSrc, delStart, delSize);
                        ...
                        if (tagName.equals(value)) {
                            for (AttributeData data : chunk.attrList) {
                                ...
                                if (attrName.equals(attrNameTemp1)) {
                                    ...
                                    int offsetStart = data.offset;
                                    ParserChunkUtils.xmlStruct.byteSrc = Utils.replaceBytes(
                                            ParserChunkUtils.xmlStruct.byteSrc, modifyData.getByte(), offsetStart);
                                    return;
                                }
                            }
                        }
    ...
    private static void modifStringChunk() {
        ...
        // 删除原始String Chunk
        ParserChunkUtils.xmlStruct.byteSrc = Utils.removeByte(ParserChunkUtils.xmlStruct.byteSrc,
                ParserChunkUtils.stringChunkOffset, Utils.byte2int(strChunk.size));
        // 插入新的String Chunk
        ParserChunkUtils.xmlStruct.byteSrc = Utils.insertByte(ParserChunkUtils.xmlStruct.byteSrc,
                ParserChunkUtils.stringChunkOffset, newStrChunkB);
    }
    ...
    public static int[] getAttrType(String tagValue) {
        ...
        if (tagValue.equals("true") || tagValue.equals("false")) {// boolean
        ...
        } else if (tagValue.equals("singleTask") || tagValue.equals("standard") || tagValue.equals("singleTop")
                || tagValue.equals("singleInstance")) {// 启动模式int类型
            ...
            String hex16String = tagValue.substring(1);
            int hex16Int = Integer.valueOf(hex16String, 16).intValue();
