...
public class StartTagChunk implements Chunk {
    ...
    public int getLen() {
        return type.length + size.length + lineNumber.length + unknown.length + uri.length + name.length + flag.length
                + attCount.length + classAttr.length + attribute.length;
    }
    ...
    public static StartTagChunk createChunk(byte[] byteSrc, int offset) {
        ...
        // 解析属性
        // 这里需要注意的是每个属性单元都是由五个元素组成，每个元素占用四个字节：namespaceuri, name, valuestring, type,
        // data
        // 在获取到type值的时候需要右移24位
